...
// Проверяет, заполненны ли все ячейки, если да, то - Ничья
bool DrawCheck(Options& options) {
	...
	for (int i = 0; i < 3; i++) //переключение по строкам
	{
		...
		{
			// Если при проверке будет обнаруженна, хоть одна свободная ячека, то ничьи не будет
	...
	// Если свободных ячеек не обнаруженно, то будет ничья
	if (drawCounter == 0)
	{
		Draw(options);
		return true;
...
void BOTRandomXY(Options& options) {
	...
	do{
		cord.X = rand() % 3;
		cord.Y = rand() % 3;
		// Если выбранные координаты уже заняты, то координаты выбираются заного, пока не будет найдено свободная ячейка
